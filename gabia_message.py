...
def stat():
	...
	dev  = __salt__['cmd.run']("find /dev/ -type b | grep -v ram | grep -v loop | awk -F/ '{print $3}'",python_shell=True)
        ...
        logfile_pattern = {'/var/log/messages':devpatten}
...
#                        string = "file: " + logfile + " not_exist, "
                ...
                tmpf = open(posfile, "w")
                # logfile , position , mtime , filesize , inode_number
                #tmpf.write("%s %s %s %s %s" % (logfile,str(p),mtime,size,ino))
